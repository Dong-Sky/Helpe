...
class myOrderDetail extends Component{
  ...
  returnButtonState = () => {
    ...
    switch(Number(status)){
      ...
      case 30:
        ...
        state.press = () => {
          Alert.alert(
            ...
            [
              ...
              {text: I18n.t('common.confirm'), onPress: () => this.operate_order('getmoney')},
            ],
            { cancelable: false }
          )
        };
        break;
      case 40:
        state.title = Number(fd)>0?I18n.t('myOrder.oa401'):I18n.t('myOrder.oa40');
        state.press = () => {
          if(Number(fd)>0){
            //已经评论
  ...
  getFeedback = () => {
    const { token, uid ,porder } = this.state;
    const url = Service.BaseUrl+Service.v+`/feedback?orderid=${porder.id}&t=${token}`;
    console.log(url);
  ...
  feedback = () => {
    ...
    const url = Service.BaseUrl+Service.v+`?t=${token}`;
    const body = `t=${token}&orderid=${order.id}&content=${content}&socre=${score}`;
    console.log(url);
